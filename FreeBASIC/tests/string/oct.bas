...
sub test_long
	...
	assert( valulng( "&o" + oct( n ) ) = TEST_VAL )
	
	for i = 1 to ((len( n ) * 8) \ 3) + 0
	    assert( valulng( "&o" + oct( n, i ) ) = TEST_VAL mod digmax )
	    digmax *= 8
	next
	
	assert( valulng( "&o" + oct( n, i ) ) = TEST_VAL )
...
sub test_int
	...
	assert( valuint( "&o" + oct( n ) ) = TEST_VAL )
	
	for i = 1 to ((len( n ) * 8) \ 3) + 0
	    assert( valuint( "&o" + oct( n, i ) ) = TEST_VAL mod digmax )
	    digmax *= 8
	next
	
	assert( valuint( "&o" + oct( n, i ) ) = TEST_VAL )
