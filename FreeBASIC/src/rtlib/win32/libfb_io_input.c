...
static void fb_hConsoleProcessKeyEvent( KEY_EVENT_RECORD *event )
{
    ...
    WORD wVsCode = event->wVirtualScanCode;
    int is_ext_code = event->uChar.AsciiChar==0;

    /* Process ENHANCED_KEY's in a different way */
    if( (event->dwControlKeyState & ENHANCED_KEY)!=0 && is_ext_code) {
        size_t i;
        ...
        unsigned uiAsciiChar = (unsigned) (unsigned char) event->uChar.AsciiChar;
        ...
        if( !AddKeyCode ) {
            if( is_ext_code ) {
                ...
                WORD wVkCode = event->wVirtualKeyCode;
