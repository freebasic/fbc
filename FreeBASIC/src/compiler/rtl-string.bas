...
data @"hex","fb_HEX_b", _
	 ...
	 NULL, FALSE, TRUE, _
	 1, _
	 FB_DATATYPE_BYTE,FB_ARGMODE_BYVAL, FALSE
...
data @"hex","fb_HEX_s", _
	 ...
	 NULL, FALSE, TRUE, _
	 1, _
	 FB_DATATYPE_SHORT,FB_ARGMODE_BYVAL, FALSE
...
function rtlStrAllocTmpResult( byval strg as ASTNODE ptr ) as ASTNODE ptr static
    ...
    '' src as string
    if( astNewPARAM( proc, strg, FB_DATATYPE_STRING ) = NULL ) then
    	exit function
    end if
...
function rtlStrToVal( byval expr as ASTNODE ptr, _
	...
	case FB_DATATYPE_BYTE, FB_DATATYPE_SHORT, FB_DATATYPE_INTEGER
...
function rtlStrChr( byval args as integer, _
    ...
    if( astNewPARAM( proc, astNewCONSTi( args, FB_DATATYPE_INTEGER ) ) = NULL ) then
    	exit function
    end if
...
function rtlStrSwap( byval str1 as ASTNODE ptr, _
    ...
    '' str1 as any
    if( astNewPARAM( proc, str1, FB_DATATYPE_STRING ) = NULL ) then
    	exit function
    end if
    ...
    '' str2 as any
    if( astNewPARAM( proc, str2, FB_DATATYPE_STRING ) = NULL ) then
    	exit function
    end if
