...
enum KWD_OPTION
    KWD_OPTION_DEFAULT 		= &h0000
	KWD_OPTION_NO_QB 		= &h0001
end enum
...
type SYMBKWD
	...
	{ _
        ...
        ( @"ULONG"      , FB_TK_ULONG       , FB_TKCLASS_KEYWORD ), _
        ...
        ( @"CULNG"      , FB_TK_CULNG       , FB_TKCLASS_KEYWORD ), _
        ...
        ( @"SIZEOF"     , FB_TK_SIZEOF      , FB_TKCLASS_QUIRKWD ), _
        ...
        ( @"DEFULNG"    , FB_TK_DEFULNG     , FB_TKCLASS_QUIRKWD ), _
        ...
        ( @"IMAGECREATE", FB_TK_IMAGECREATE	, FB_TKCLASS_QUIRKWD ), _
        ( NULL ) _
	}
...
'':::::
sub symbKeywordInit( )
    dim as integer i = any, do_add = any

	i = 0
	do until( kwdTb(i).name = NULL )

        '' don't add if '-lang qb' is set.
        if( kwdTb(i).opt = KWD_OPTION_NO_QB ) then
