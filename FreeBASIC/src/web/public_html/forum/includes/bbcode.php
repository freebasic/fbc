...
$bbcode_tpl = null;
	...
	require_once( $phpbb_root_path . '../geshi/geshi.php' );
...
function bbencode_second_pass_code($text, $uid, $bbcode_tpl)
{
	static $code_entities_match = array('/&amp;/', '/&lt;/', '/&gt;/', '/&quot;/', '/&#58;/', '/&#91;/', '/&#93;/', '/&#40;/', '/&#41;/', '/&#123;/', '/&#125;/');
	static $code_entities_replace = array('&', '<', '>', '"', ':', '[', ']', '(', ')', '{', '}');
	...
	$geshi =& new GeSHi( '', 'qbasic', $phpbb_root_path . '../geshi/geshi/' );
	$geshi->set_header_type( GESHI_HEADER_DIV );
	$geshi->enable_classes( true );

	for ($i = 0; $i < $match_count; $i++)
	{
		$body =& preg_replace( $code_entities_match, $code_entities_replace, $matches[1][$i] );
		
		$geshi->set_source( $body );

		$str_to_match = "[code:1:$uid]" . $matches[1][$i] . "[/code:1:$uid]";
