...
Type KEYBOARD_DRIVER
	...
	exit As Sub CDecl()
	poll As Sub CDecl()
	set_leds As Sub CDecl(ByVal leds As Integer)
	set_rate As Sub CDecl(ByVal delay As Integer, ByVal rate As Integer)
	wait_for_input As Sub CDecl()
	stop_waiting_for_input As Sub CDecl()
	scancode_to_ascii As Function CDecl(ByVal scancode As Integer) As Integer
End Type
...
Extern Import keyboard_driver Alias "keyboard_driver" As KEYBOARD_DRIVER Ptr
Extern Import _keyboard_driver_list Alias "_keyboard_driver_list" As _DRIVER_INFO Ptr
