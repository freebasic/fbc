...
sub emitBegin( )
	...
	emitLine( "" )
	emitLine( "#include ""fbcu.bi""" )
	emitLine( "" )
	emitLine( "namespace fbc_tests.optimizations.math_torture" )
	emitLine( "" )
	emitLine( "sub test" & ctx.filecount & " cdecl( )" )
	emitIndent( )
...
sub emitEnd( )
	...
	emitLine( "end sub" )
	emitLine( "" )
	emitLine( "private sub ctor( ) constructor" )
	emitIndent( )
	emitLine( "fbcu.add_suite( ""fbc_tests.optimizations.math-torture-" & ctx.filecount & """ )" )
	emitLine( "fbcu.add_test( ""test" & ctx.filecount & """, @test" & ctx.filecount & " )" )
	emitUnindent( )
	emitLine( "end sub" )
	emitLine( "" )
	emitLine( "end namespace" )
