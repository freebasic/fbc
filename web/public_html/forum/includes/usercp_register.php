...
$unhtml_specialchars_replace = array('>', '<', '"', '&');
	...
	define( 'SPAMMER_WORDS', '/(.*?xanax.*?|.*?viagra.*?|.*?casino.*?|.*?porn.*?|.*?girls.*?|.*?tv.*?plasma.*?|.*?plasma.*?tv.*?|.*?sony.*?|.*?cigar.*?|.*?pharmacy.*?|.*?pills.*?|.*?\-online.*?|.*?rolex.*?)/msi' );

function check_spammer( &$text )
{
		if( preg_match( SPAMMER_WORDS, $text ) )
			return TRUE;
		else
			return FALSE;
}
	...
	$mode == 'register' )
{
	...
	/* catch spammers, -12 GMT shouldn't happen, NZ = -11 */
	if( $user_timezone == -12 )
	{
		$error = true;
		$error_msg = $lang['Fields_empty'];
...
if ( isset($HTTP_POST_VARS['submit']) )
{
	...
	if ( $email != $userdata['user_email'] || $mode == 'register' )
	{
		...
		if( !$error ) 
		{
			if( check_spammer( $email ) )
			{
				$error = TRUE;
				$error_msg .= ( ( isset($error_msg) ) ? '<br />' : '' ) . $lang['Error_login'];
			}
		}
