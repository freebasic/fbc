/* UTF-encoded file devices open (wide string) */
...
static int hCheckBOM_W( FB_FILE *handle )
{
    int res, bom = 0;
...
static int hWriteBOM_W( FB_FILE *handle )
{
    int bom;
    FILE *fp = (FILE *)handle->opaque;
