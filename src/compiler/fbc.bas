...
type FBCCTX
	...
	emitstep2only			as integer  '' -gen gcc: write out final asm file only (generated by gcc)
	preservestep2asm		as integer  '' -gen gcc: preserve final asm
...
private sub handleOpt(byval optid as integer, byref arg as string)
	...
	case OPT_RR
		fbSetOption( FB_COMPOPT_OUTTYPE, FB_OUTTYPE_OBJECT )
		fbc.emitstep2only = TRUE
		fbc.preservestep2asm = TRUE
...
private function hCompileStep2Module( byval module as FBCIOFILE ptr ) as integer
	...
	if( fbc.preservestep2asm = FALSE ) then
		fbcAddTemp( asmfile )
	end if
...
private sub hPrintOptions( )
	...
	print "  -rr              Like -r, but for -gen gcc's final .asm"
